new Vue({
    ...
    created() {
        ...
        window.history.replaceState('', document.title, "/cheat/u/" + vm.userid);
    },
    methods: {
        LoadProfileData(userid) {
            var vm = this;
            vm.$axios.get("https://" + window.location.hostname + ":" + window.location.port + "/api/get_user_cheat", {
                params: {
                    id: userid,
                }
            })
        ...
        LoadMostBeatmaps(userid, mode, mods) {
            var vm = this;
            vm.$axios.get("https://" + window.location.hostname + ":" + window.location.port + "/api/get_most_beatmaps_cheat", {
                params: {
                    id: userid,
                    mode: mode,
                    mods: mods,
        ...
        LoadScores(userid, mode, mods, sort) {
            ...
            vm.$axios.get("https://" + window.location.hostname + ":" + window.location.port + "/api/get_scores_cheat", {
                params: {
                    id: userid,
                    mode: mode,
                    mods: mods,
        ...
        LoadGrades(userid, mode, mods) {
            var vm = this;
            vm.$axios.get("https://" + window.location.hostname + ":" + window.location.port + "/api/get_grade_cheat", {
                params: {
                    id: userid,
                    mode: mode,
                    mods: mods,
        ...
        LoadReplay(id, mods) {
            ...
            vm.$axios.get(window.location.protocol + "//" + window.location.hostname + ":" + window.location.port + "/api/get_replay_cheat",
                {
                    params: {
                        id: id,
                        mods: mods,
