new Vue({
    ...
    methods: {
        LoadLeaderboard(sort, mode, mods) {
            ...
            vm.mods = mods;
            vm.sort = sort;
            window.history.replaceState('', document.title, "/cleaderboard/" + vm.mode + "/" + vm.sort + "/" + vm.mods);
            vm.$axios.get("https://" + window.location.hostname + ":" + window.location.port + "/api/get_leaderboard_cheat", { params: { 
                mode: mode, 
                sort: sort, 
                mods: mods,
            }})
