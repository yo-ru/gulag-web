...
async def get_leaderboard():
    ...
    if country:
        res = await glob.db.fetchall(
            f"SELECT ROW_NUMBER() OVER () AS id, users.id AS userid, users.name AS username, users.country, tscore_{mods}_{mode} AS tscore, rscore_{mods}_{mode} AS rscore, pp_{mods}_{mode} AS pp, "
            f"plays_{mods}_{mode} AS plays, playtime_{mods}_{mode} AS playtime, acc_{mods}_{mode} AS acc, maxcombo_{mods}_{mode} AS maxcombo "
            f"FROM stats "
            f"JOIN users "
            f"ON stats.id = users.id "
