...
async def home():
    ...
    vnc = await glob.db.fetch('SELECT pp, users.name FROM scores_vn_cheat LEFT JOIN users ON scores_vn_cheat.userid = users.id LEFT JOIN maps ON scores_vn_cheat.map_md5 = maps.md5 WHERE users.priv & 1 AND users.frozen = 0 AND scores_vn_cheat.status = 2 AND scores_vn_cheat.mode = 0 ORDER BY pp DESC LIMIT 1')
    rxc = await glob.db.fetch('SELECT pp, users.name FROM scores_rx_cheat LEFT JOIN users ON scores_rx_cheat.userid = users.id LEFT JOIN maps ON scores_rx_cheat.map_md5 = maps.md5 WHERE users.priv & 1 AND users.frozen = 0 AND scores_rx_cheat.status = 2 AND scores_rx_cheat.mode = 0 ORDER BY pp DESC LIMIT 1')
    apc = await glob.db.fetch('SELECT pp, users.name FROM scores_ap_cheat LEFT JOIN users ON scores_ap_cheat.userid = users.id LEFT JOIN maps ON scores_ap_cheat.map_md5 = maps.md5 WHERE users.priv & 1 AND users.frozen = 0 AND scores_ap_cheat.status = 2 AND scores_ap_cheat.mode = 0 ORDER BY pp DESC LIMIT 1')
    try:
